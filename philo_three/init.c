...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/03/28 16:24:08 by juyang            #+#    #+#             */
/*   Updated: 2021/03/28 16:24:09 by juyang           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int					semaphore_init(t_prog *prog)
{
	...
	if (prog->system.forks == SEM_FAILED)
		return (-1);
	prog->system.status = sem_open("status", O_CREAT, 0644, 0);
	if (prog->system.status == SEM_FAILED)
		return (-1);
	prog->system.write = sem_open("write", O_CREAT, 0644, 1);
	if (prog->system.write == SEM_FAILED)
		return (-1);
	prog->system.finish_check = sem_open("finish_check", O_CREAT, 0644, 0);
